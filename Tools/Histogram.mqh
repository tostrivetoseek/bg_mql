#ifndef TOOLS_HISTOGRAM_MQH
#define TOOLS_HISTOGRAM_MQH
...
//+------------------------------------------------------------------+
//| from Class Canvas\Charts\CHistogramChart                         |
//| Usage: generates histogram chart                                 |
//+------------------------------------------------------------------+
class Histogram : public CChartCanvas
  {
...
public:
   ...
   void SeriesAddTitles(const string &titles[])
   {
       m_bar_titles_count = ArraySize(titles);
       ArrayCopy(m_bar_titles, titles);
   }
  };
//+------------------------------------------------------------------+
//| Constructor                                                      |
//+------------------------------------------------------------------+
Histogram::Histogram(void) : m_gradient(true),
                               m_bar_gap(3),
                               m_bar_min_size(5),
                               m_bar_border(0),
                               m_bar_titles_count(0)
  {
...
//+------------------------------------------------------------------+
bool Histogram::Create(const string name,const int width,const int height,ENUM_COLOR_FORMAT clrfmt)
  {
...
//| Adds data series                                                 |
//+------------------------------------------------------------------+
bool Histogram::SeriesAdd(const double &value[],const string descr,const uint clr)
  {
//--- check
...
//| Inserts data series                                              |
//+------------------------------------------------------------------+
bool Histogram::SeriesInsert(const uint pos,const double &value[],const string descr,const uint clr)
  {
//--- check
   if(m_data_total==m_max_data)
      return(false);
...
//| Updates data series                                              |
//+------------------------------------------------------------------+
bool Histogram::SeriesUpdate(const uint pos,const double &value[],const string descr,const uint clr)
  {
//--- check
   if(pos>=m_data_total)
      return(false);
...
//| Deletes data series                                              |
//+------------------------------------------------------------------+
bool Histogram::SeriesDelete(const uint pos)
  {
//--- check
...
//+------------------------------------------------------------------+
bool Histogram::ValueUpdate(const uint series,const uint pos,double value)
  {
...
//| Draws histogram                                                  |
//+------------------------------------------------------------------+
void Histogram::DrawData(const uint idx)
  {
   double value=0.0;
//--- check
      ...
      if(IS_SHOW_VALUE)
        {
         ...
         if (i < m_bar_titles_count) {
            text = m_bar_titles[i] + ": " + text;
         }
...
//| Draws bar                                                        |
//+------------------------------------------------------------------+
void Histogram::DrawBar(const int x,const int y,const int w,const int h,const uint clr)
  {
//--- draw bar
...
//+------------------------------------------------------------------+
void Histogram::GradientBrush(const int size,const uint fill_clr)
  {
